...
from .xflux.src.flux.util import (
    load_safetensors,
    load_controlnet,
)
...
from .utils import (
    FirstHalfStrengthModel,
    FluxUpdateModules,
    LinearStrengthModel,
    SecondHalfStrengthModel,
    SigmoidStrengthModel,
    attn_processors,
    is_model_pathched,
    merge_loras,
    LATENT_PROCESSOR_COMFY,
    ControlNetContainer,
    comfy_to_xlabs_lora,
    check_is_comfy_lora,
    patchify,
...
folder_paths.folder_names_and_paths["xlabs"] = (
    [dir_xlabs],
    folder_paths.supported_pt_extensions,
)
folder_paths.folder_names_and_paths["xlabs_loras"] = (
    [dir_xlabs_loras],
    folder_paths.supported_pt_extensions,
)
folder_paths.folder_names_and_paths["xlabs_controlnets"] = (
    [dir_xlabs_controlnets],
    folder_paths.supported_pt_extensions,
)
folder_paths.folder_names_and_paths["xlabs_ipadapters"] = (
    [dir_xlabs_ipadapters],
    folder_paths.supported_pt_extensions,
)
folder_paths.folder_names_and_paths["xlabs_flux"] = (
    [dir_xlabs_flux],
    folder_paths.supported_pt_extensions,
)
folder_paths.folder_names_and_paths["xlabs_flux_json"] = (
    [dir_xlabs_flux],
    set(
        {
            ".json",
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "model": ("MODEL",),
                "lora_name": (
                    cleanprint(folder_paths.get_filename_list("xlabs_loras")),
                ),
                "strength_model": (
                    "FLOAT",
        ...
        if debug:
            ...
            try:
                print(
                    get_attr(
                        tyanochky,
                        "diffusion_model.double_blocks.0.processor.lora_weight",
                    )
                )
            except:
                pass
        ...
        if debug:
            try:
                if isinstance(
                    ...
                    pedovki = get_attr(
                        tyanochky, "diffusion_model.double_blocks.0.processor"
                    ).lora_weight
                    if len(pedovki) > 0:
...
class LoadFluxControlNet:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "controlnet_path": (
                    folder_paths.get_filename_list("xlabs_controlnets"),
                ),
            }
        }
    ...
    def loadmodel(self, model_name, controlnet_path):
        ...
        checkpoint = load_checkpoint_controlnet(
            os.path.join(dir_xlabs_controlnets, controlnet_path)
        )
        if checkpoint is not None:
...
class ApplyFluxControlNet:
    ...
    def prepare(self, controlnet, image, strength, controlnet_condition=None):
        ...
        controlnet_image = (
            controlnet_image.permute(0, 3, 1, 2).to(torch.bfloat16).to(device)
        )
...
class XlabsSampler:
    ...
    def sampling(
        ...
        x = get_noise(
            bc, height, width, device=device, dtype=dtype_model, seed=noise_seed
        )
        orig_x = None
        if c == 16:
        ...
        inp_cond = prepare(
            conditioning[0][0], conditioning[0][1]["pooled_output"], img=x
        )
        ...
        if controlnet_condition is None:
            ...
            cnet_conditions = [
                prepare_controlnet_condition(el) for el in controlnet_condition
            ]
            containers = []
            for el in cnet_conditions:
                start_step = int(el["start"] * len(timesteps))
                end_step = int(el["end"] * len(timesteps))
                container = ControlNetContainer(
                    el["model"],
                    el["img"],
                    el["controlnet_strength"],
                    start_step,
                    end_step,
                )
    ...
    def masked_sampling(
        ...
        for i in range(len(conditioning)):
            ...
            mask = mask.unsqueeze(1).repeat(
                x.shape[0] // mask.shape[0], x.shape[1], 1, 1
            )

            inp_cond["mask"] = patchify(mask)
...
class LoadFluxIPAdapter:
    ...
    def loadmodel(self, ipadatper, clip_vision, provider):
        ...
        ret_ipa["double_blocks"] = torch.nn.ModuleList(
            [IPProcessor(4096, 3072) for i in range(19)]
        )
...
class ApplyFluxIPAdapter:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "ip_scale": (
                    "FLOAT",
                    {"default": 0.93, "min": 0.0, "max": 1.0, "step": 0.001},
                ),
    ...
    def applymodel(self, model, ip_adapter_flux, image, ip_scale):
        ...
        # TYANOCHKYBY=16
        ip_projes_dev = next(
            ip_adapter_flux["ip_adapter_proj_model"].parameters()
        ).device
...
class ApplyAdvancedFluxIPAdapter:
    ...
    def applymodel(
        ...
        if smothing_type == "Linear":
            ...
            strength_model = SecondHalfStrengthModel(
                begin_strength, end_strength, count
            )
        ...
        for i, block in enumerate(ip_adapter_flux["double_blocks"]):
            ipad = IPProcessor(
                block.context_dim, block.hidden_dim, ip_projes, strength_model[i]
            )
            ipad.load_state_dict(block.state_dict())
            ipad.in_hidden_states_neg = ip_neg_pr
            ipad.in_hidden_states_pos = ip_projes
...
NODE_CLASS_MAPPINGS = {
    ...
    "XlabsSamplerWithMask": XlabsSamplerWithMask,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "XlabsSamplerWithMask": "Xlabs Sampler with Mask Conditioning",
